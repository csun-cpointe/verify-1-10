...
startup() {
  ...
  if [ $? -ne 0 ]; then
    helm upgrade --install $INFRA_NAME verify-1-10-infrastructure \
      --values verify-1-10-infrastructure/values.yaml \
      --values verify-1-10-infrastructure/values-dev.yaml
    if ! kubectl wait -A --for=condition=Ready pod -l app.kubernetes.io/name=argocd-server; then
      exit $?
    fi
    argocd repo add https://github.com/csun-cpointe/verify-1-10.git --server localhost:30080 --plaintext --insecure-skip-server-verification
  fi
}
...
deploy() {
  ...
  if is_app_running; then
    echo "verify-1-10 is deployed"
  else
    branch=$(git rev-parse --abbrev-ref HEAD)
    echo "Deploying verify-1-10 from branch '$branch'..."
    argocd app create $APP_NAME \
      --server localhost:30080 --plaintext \
      --dest-namespace argocd \
      --dest-server https://kubernetes.default.svc \
      --repo https://github.com/csun-cpointe/verify-1-10.git \
      --path verify-1-10-deploy/src/main/resources \
      --revision $branch \
...
down() {
  if is_app_running; then
    ...
    echo "verify-1-10 is not deployed"
  fi
}
